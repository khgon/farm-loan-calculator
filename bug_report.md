...
## 최근 수정사항 한눈에 보기
- **중복 import 제거**: `app.py` 안에서 이미 전역에 불러온 `Decimal` 관련 모듈을 다시 부르던 부분을 없애 깔끔하게 정리했습니다.
- **원금 분배 방식 다듬기**: 나눗셈의 나머지를 `divmod`로 계산해 남은 원금이 연차별로 자연스럽게 배분되도록 했습니다.
- **requirements.txt 정리**: 패키지 이름 앞의 공백을 지워 `pip install -r requirements.txt` 명령이 문제없이 동작합니다.
